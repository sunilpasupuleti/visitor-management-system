...
async function rangeData() {
  ...
  if (!from || !to) {
    ...
    if (result) {
      let data = result;
      visitorMeetings = data.meetings;
      AllVisitorMeetings = data.meetings;
...
async function fetchUserData() {
  const api_url = URL + "/visitor/getVisitorDetails?visitorId=" + visitorId;
...
function sortByFilter() {
  let sortByEmp = $("#employee-name").val();
  let sortByStatus = $("#status").val();
  console.log(sortByEmp, sortByStatus);
  let filteredData;

  let isEmpVal = true;

  if (sortByEmp === "" || sortByEmp === "default") {
    isEmpVal = false;
  }

  filteredData = AllVisitorMeetings.filter((e) => {
    let sortStatus;
    if (sortByStatus === "completed") {
      let checkString = e.status === "completed" && !e.isInProgress;
      sortStatus = isEmpVal
        ? e.employee.name === sortByEmp && checkString
        : checkString;
      ...
      sortStatus = isEmpVal ? e.employee.name === sortByEmp : true;
    }

    return sortStatus;
  });

  visitorMeetings = filteredData;
  buildMeetingsTable();
}
...
async function buildUserData(data) {
  ...
  var date = convertDate(createdAt, "MMM DD ,  YYYY");
  $("#name").val(name);
  $("#phone").val(phone);
  $("#email").val(email);
  $("#company").val(company);
  ...
  $("#addedon").val(date);
  $("#idType").html(idType);
  $("#idLink").attr("href", idLink);
  $("#selfieLink").attr("href", selfieLink);
...
function buildMeetingsTable() {
  ...
  tableBody.empty();
  const meetings = visitorMeetings;
  if (meetings == 0) {
    tableBody.html(
...
function returnVariables(m) {
  ...
  data.gateExitTime = convertDate(data.ateExitTime, "MMM DD , YYYY - hh:mm a ");
...
function openMeetingDetailsModal(mid) {
  ...
  const data = visitorMeetings.filter((e) => e._id === mid)[0];
