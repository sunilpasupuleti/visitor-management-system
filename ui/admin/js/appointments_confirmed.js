var appointmentType = "getConfirmedOrders";
...
function appointments_confirmed() {
  /* Using Temporary api call , replacw with proper api call */
  fetch(`https://api.notarizetech.com/admin/order/getInProgressOrders`, {
      method: "POST",
      body: JSON.stringify({
    ...
    .then((response) => {
      ...
      buildAppointmentconfirmedTable(response);
      hideLoader()
    })
    .catch((err) => {
      swal({
...
function buildAppointmentconfirmedTable(tableData) {
  console.log(tableData)
  let tableBody = $(".appointment-confirmed");
  tableBody.empty();
  const {
    pageCount,
    orderCount,
      ...
      let tablerow = `
        ...
        <th><a style="cursor:pointer; color:#0341fc;" href="order_details.html?orderId=${order._id}&ecscrow=${order.appointment.escrowNumber}">${order.appointment.escrowNumber}</a></th>
            ...
            <td>
              ...
              <span>Confirmed At :${getDate(order.confirmedAt)}</span>
            </td>
        </tr>`;
...
function getPageButtons(pageCount, tableName) {
  ...
  $(".page-button-number").on("click", (e) => {
    ...
    appointments_confirmed(reqData);
