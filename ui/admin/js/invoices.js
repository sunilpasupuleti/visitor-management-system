...
function showTransactions(typeOfTransaction, i) {
  if (typeOfTransaction == "pending") {
    ...
    fetch(
      `https://sideshnew-zenwin.herokuapp.com/admin/payment/rechargeRequestList?limit=10&skip=` +
        s,
      {
        method: "GET",
        // body: JSON.stringify({
    ...
    fetch(
      `https://sideshnew-zenwin.herokuapp.com/admin/payment/rechargeRecords?limit=10&skip=` +
        s1 +
        `&status=approved`,
      {
        method: "GET",
...
function buildRejectTable(tableData) {
  ...
  if (tableData.length == 0) {
    tableBody.html(
      '<h2 class="d-flex justify-content-center mt-3">No more rejected recharges</h2>'
    );
  } else {
    tableData.forEach((invoice) => {
      ...
      tablerow += `<tr>
          <th>
             <span class="form-control-label ml-2"><a style="cursor:pointer; color:#0341fc;" href="client_profile.html?customerId=${invoice.userId}">${invoice.Id}</span>
      </th>
...
function buildApproveTable(tableData) {
  ...
  tableBody.empty();
  const { pageCount, invoiceCount, pageNumber, invoices } = tableData;
  if (tableData.length == 0 || tableData.error) {
    tableBody.html(
      '<h2 class="d-flex justify-content-center mt-3">No more approved recharges</h2>'
    );
  } else {
    tableData.forEach((invoice) => {
      let tablerow = "";
...
function buildInvoiceTable(tableData) {
  ...
  if (tableData.length == 0) {
    tableBody.html(
      '<h2 class="d-flex justify-content-center mt-3">No more pending recharges</h2>'
    );
  } else {
    tableData.forEach((invoice) => {
      let tablerow = "";
